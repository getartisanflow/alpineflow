...
- Applies dimensions from `node.dimensions` — inline `style.height` is only set on container nodes (those with `childLayout`) or when `fixedDimensions: true`; leaf nodes let content determine height and the ResizeObserver captures the natural height back into `node.dimensions`
