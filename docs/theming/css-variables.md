---
title: CSS Variables
description: Complete variable reference for theming.
order: 1
---
...
```js
// Switch to a red accent at runtime
const container = document.querySelector('.flow-container');
container.style.setProperty('--flow-node-selected-border-color', '#dc2626');
container.style.setProperty('--flow-edge-stroke-selected', '#dc2626');
container.style.setProperty('--flow-handle-active-bg', '#dc2626');
```
...
AlpineFlow uses a structured z-index system for element stacking. These values are hardcoded in structural CSS and are not themeable.
